...
#
# 
# a set of scripts runnable on an external computer, doin' the ec2-tools 
...
ec2-describe-instances $aws_instance > tempfile2.txt
...
ec2-create-image $aws_instance -n "MWiki14_replicated $(date +%s)" -d "An image generated by create_aws_replica.sh" > tempfile3.txt
...
ec2-run-instances -z$aws_availability_zone -k$aws_key_name -t$aws_instance_type -g$aws_security_group $aws_replica_image > tempfile4.txt
...
sleep 60
ec2-describe-instances $aws_replica_instance > tempfile5.txt
...
#TODO: Maybe we need to clean up the AMI generated?
