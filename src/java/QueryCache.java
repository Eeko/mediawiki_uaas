      ...
      switch (statement.getState()) { 
        case Statement.PIPELINE_PROCESSING:
          if (statement.getStatement().toLowerCase().startsWith("select")) 
            statement.setStatement("CALL cacheLookup(’" + statement.getRequest().getId() + "’, ’" + statement.getStatement() + "’)"); 
    ...
    try {
      switch (database.getContextState()) {
        ...
        case Database.DATABASE_UP: 
          ...
          java.sql.Statement s = c.createStatement(); 
          s.execute("CREATE PROCEDURE cacheLookup(reqid VARCHAR(10), query VARCHAR(100))" + "PARAMETER STYLE JAVA LANGUAGE JAVA READS SQL DATA DYNAMIC RESULT SETS 1" + "EXTERNAL NAME ’gorda.demo.QueryCache.cacheLookup’"); 
          s.close(); 
          c.close();
          database.continueExecution(); 
          break;
